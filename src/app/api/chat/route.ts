...
function checkAndUpdateRateLimit(clientIP: string): { allowed: boolean; message?: string } {
  ...
  if (userData.count >= MAX_USER_REQUESTS_PER_DAY) {
    return {
      allowed: false,
      message: 'Daily message limit reached (100 messages per day). Please try again tomorrow.'
    }
  }
...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.log('🔑 Checking API key - exists:', !!process.env.GOOGLE_GENERATIVE_AI_API_KEY)
    if (!process.env.GOOGLE_GENERATIVE_AI_API_KEY) {
      console.log('❌ API key not found')
      return Response.json(
        { error: 'Google Gemini API key is not configured. Please add GOOGLE_GENERATIVE_AI_API_KEY to your environment variables.' },
        { status: 500 }
      )
    }
    ...
    const result = await streamText({
      ...
      tools: {
        ...
        getTechStack: tool({
          ...
          parameters: z.object({
            category: z.string().optional().describe('Filter by category (e.g., Framework, Library, etc.)'),
          }),
        ...
        getExperience: tool({
          ...
          execute: async ({ company }) => {
            const filteredExp = company
              ? EXPERIENCES.filter(exp => exp.companyName.toLowerCase().includes(company.toLowerCase()))
              : EXPERIENCES
    ...
    if (error instanceof Error) {
      if (error.message.includes('API key')) {
        return Response.json(
          { error: 'Invalid or missing API key. Please check your Google Gemini API configuration.' },
          { status: 401 }
        )
      }
